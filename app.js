...
async function loadPuzzle() {
  ...
  try {
    ...
    const sanitizedWords = Array.isArray(puzzlePayload?.words) && puzzlePayload.words.length
      ? puzzlePayload.words
      : [...DEFAULT_PUZZLE.words];
    if (sanitizedWords !== puzzlePayload?.words) {
      log.warn('Puzzle payload missing usable words, falling back to defaults');
    }

    state.targetWords = sanitizedWords.map((word) => word.toLowerCase());
    state.insight = puzzlePayload?.insight || DEFAULT_PUZZLE.insight;
    state.theme = puzzlePayload?.theme || DEFAULT_PUZZLE.theme;
    state.found.clear();
    state.selected = [];
    state.hintsUsed = 0;
    state.streak = 0;
